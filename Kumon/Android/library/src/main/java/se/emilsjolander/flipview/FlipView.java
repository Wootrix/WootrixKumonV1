...
public class FlipView extends FrameLayout
{
	...
	public interface OnOverFlipListener
	{
		public void onOverFlip(FlipView v, OverFlipMode mode, boolean overFlippingPrevious, float overFlipDistance,
				float flipDistancePerPage, int position);
	}
	...
	public FlipView(Context context, AttributeSet attrs, int defStyle)
	{
		...
		mIsFlippingVertically = a.getInt(R.styleable.FlipView_fliporientation, VERTICAL_FLIP) == VERTICAL_FLIP;
	...
	private void dataSetChanged()
	{
		...
		if (mAdapter.hasStableIds() && currentPage != INVALID_PAGE_POSITION)
		{
			...
			if (currentPage == INVALID_PAGE_POSITION)
			{
				newPosition = 0;
			}
					...
					if (isOverFlipping)
					{
						...
						if (mOnOverFlipListener != null)
						{
							...
							mOnOverFlipListener.onOverFlip(this, mOverFlipMode, overFlip < 0, Math.abs(overFlip),
									FLIP_DISTANCE_PER_PAGE, mCurrentPageIndex);
						}
					}
					else
						if (mIsOverFlipping)
						{
							...
							if (mOnOverFlipListener != null)
							{
								...
								mOnOverFlipListener.onOverFlip(this, mOverFlipMode, false, 0, FLIP_DISTANCE_PER_PAGE, mCurrentPageIndex);
								mOnOverFlipListener.onOverFlip(this, mOverFlipMode, true, 0, FLIP_DISTANCE_PER_PAGE,mCurrentPageIndex);
							}
						}
				}
				break;
	...
	private void setDrawWithLayer(View v, boolean drawWithLayer)
	{
		if (isHardwareAccelerated())
		{
			if (v.getLayerType() != LAYER_TYPE_HARDWARE && drawWithLayer)
			{
				...
				if (v.getLayerType() != LAYER_TYPE_NONE && !drawWithLayer)
				{
