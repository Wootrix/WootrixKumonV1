...
class RSSParser {
	...
	function parse()
	{
		...
		if (!isset($rawFeed))
		{
			$rawFeed = utf8_encode(file_get_contents($this->feed_uri));
                        
		}
                
		$xml = new SimpleXmlElement($rawFeed,TRUE);
		...
		if ($xml->channel)
		{
			...
			{ 
				...
				$data['link'] = (string)$item->link;
                                ...
                                $data['thumnail_url'] = $item->children('http://search.yahoo.com/mrss/')->thumbnail->attributes();
                                $data['content_url'] = $item->children('http://search.yahoo.com/mrss/')->content->attributes();
                                $data['enclosure']   = $item->enclosure;
                                $dc = $item->children('http://purl.org/dc/elements/1.1/');
                                $data['creator'] = $dc->creator;
                                $data['date']    =  $dc->date;
                                $content = $item->children('content', 'http://purl.org/rss/1.0/modules/content/');
                                $data['content_encoded'] = $content->encoded;
                                $pub = $item->children('http://www.w3.org/2005/Atom');
                                $data['publish']  = $pub->updated; 
                                $author = $item->children('http://www.w3.org/2005/Atom');
                                $data['author1'] = $author->author->name; 
                                
                                //$data['media:group'] = (string)$item->mediaCount;
                                
                                //$data['media:group'] = $item->children($namespaces['media:group'])->content->attributes()->url;
				/*START CROLING FOR MEDIA CONTENT */
//                                
//                                $media = $item->children( 'http://search.yahoo.com/mrss/' );
//                                $namespaces = $item->getNameSpaces( true );
//                                $media = $item->children( $namespaces['group'] );
