...
<script>
    ...
    function LanguageData(val,catId,langId,catlang) {
        window.location.href="<?php echo base_url();?>index.php/getpublishedarticlelist?catagory="+catId+"&lang="+val+"&cat_lang=" +catlang;
    }
  ...
  /* SORT CATAGORY DATA*/
   ...
   function CatagoryData(val,catId,langId,catlang) {
        var rowId = val;//      
        window.location.href="<?php echo base_url();?>index.php/getpublishedarticlelist?catagory="+val+"&lang="+langId+"&cat_lang=" +catlang;
...
<nav class="clearfix">
    <div class="fix-container clearfix">
       <ul>
            ...
            <li class="active"><a href="getpublishedarticlelist"><?php echo $this->lang->line('Published') ?> <span class="green"><?php echo $all_publish_article; ?></span></a></li>            
...
<!--
            ...
            <div class="search-box">
                <div class="language-filter clearfix">
                    <div class="select-container">
                        ...
                        <form id="languge_form" action="<?php echo $this->config->base_url() ?>index.php/getpublishedarticlelist" method="POST">
                        <select class="choose-lang" onChange="LanguageData(this.value,'<?php echo $_GET['catagory'] ?>','<?php echo $_GET['lang'] ?>','<?php echo $_GET['cat_lang'] ?>');" name="lang" id="lang">
                            <option value="" name=""><?php echo $this->lang->line('change_language') ?></option>
            ...
            <div class="language-filter clearfix">
                <div class="select-container">
                    <div class="select-arrow">
                     ...
                     <form id="languge_form" action="<?php echo $this->config->base_url() ?>index.php/getpublishedarticlelist" method="POST">
                        <select class="choose-lang" onChange="CatLangData(this.value, '<?php echo $_GET['catagory'] ?>', '<?php echo $_GET['cat_lang'] ?>', '<?php echo $_GET['lang'] ?>');" name="cat_lang" id="lang">
        ...
        <div class="search-bar right-align-box">
            <div class="language-filter clearfix">
                <div class="select-container">
                    ...
                    <form id="catagory_form" action="<?php echo $this->config->base_url() ?>index.php/getpublishedarticlelist" method="POST">
                            <select class="choose-lang" onChange="CatagoryData(this.value,'<?php echo $_GET['catagory'] ?>','<?php echo $_GET['lang'] ?>','<?php echo $_GET['cat_lang'] ?>');" name="catagory" id="catagory">
    ...
    <div class="category-list">
        ...
        <script>
    $(document).ready(function () {
        $('#publishedArticle').dataTable({"bPaginate": false,"bSort": false});
    });
        </script>
            <table id="publishedArticle" border="0" cellpadding="0" cellspacing="0">
                <thead>
                 <tr>
...
-->                            <td><?php if($value['article_language']=='1'){ echo"English"; }
                        ...
                        <?php } elseif ($value['created_by'] == '0') /* CREATED BY CROLLER */ { ?>                                    
                            <?php if ($value['status'] == '1') { ?>
                                <spam class="rejected-btn"><?php echo $this->lang->line("NEW"); ?></spam>
                            <?php } elseif ($value['status'] == '2') { ?>
                        ...
                        ?>
                             ...
                             <td>
                            <?php if ($value['status'] == '1') { ?>
                                <a class="aprove-btn" href="#" onclick="getAllSupportDetails('<?php echo $value['id']; ?>');">Publish</a>
