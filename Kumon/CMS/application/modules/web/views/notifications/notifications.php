...
<div class="fix-container clearfix">
    ...
    <div class="notification-container">
        ...
        <form name="push_notify" action="" method="post">
...
<script>
    ...
    var newFilter = function(){
        ...
        $.ajax({
            url: '<?php echo $this->config->base_url();?>index.php/newFilter',
            type: 'POST',
            data: {
    ...
    $('#fld_top_0').multipleSelect({
        position: 'bottom'
    });

    function multisel(fid) {
        $('#fld_top_' + fid).multipleSelect({
            position: 'bottom'
        });
    }
    ...
    //Ajax for Message Type
    $(document).on("change", ".fld_msgtype", function (e) {
        ...
        if (typeval != "message") {
            if (typeval == "closemagazine") {
                $('#div_customer_' + formval).hide();
            }
            else {
                $('#div_customer_' + formval).show();
            }
            if (typeval != "closemagazine") {
                $('#div_cmtyp_' + formval).html('');
            }
            $.ajax({
                url: '<?php echo $this->config->base_url();?>index.php/notify_ajax',
                type: 'POST',
                data: 'type=mtype&msg=' + typeval + '&form=' + formval,
                success: function (data) {
                    //called when successful
                    $('#div_msgtyp_' + formval).html(data);
                }
            });
        }
        else {
            $('#div_msgtyp_' + formval).html('');
            $('#div_cmtyp_' + formval).html('');
