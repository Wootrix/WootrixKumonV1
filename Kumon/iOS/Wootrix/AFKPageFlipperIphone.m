...
@implementation AFKPageFlipperIphone
...
- (void) initFlip
{
	...
	if (flipDirection == AFKPageFlipperDirectionRightIphone) {
	...
	blankFlipAnimationLayerOnLeft1.anchorPoint = CGPointMake(0.5, 1.0);
	...
	blankFlipAnimationLayerOnRight1.anchorPoint = CGPointMake(0.5, 1.0);
	...
	blankFlipAnimationLayerOnLeft2.anchorPoint = CGPointMake(0.5, 1.0);
	...
	blankFlipAnimationLayerOnRight2.anchorPoint = CGPointMake(0.5, 1.0);
	...
	blankFrontLayerForLayerLeft.transform = CATransform3DMakeRotation(M_PI, 1.0, 0, 0);
	...
	blankFrontLayerForLayerRight.transform = CATransform3DMakeRotation(M_PI, 1.0, 0, 0);
	...
	blankFrontLayerForLayerLeft2.transform = CATransform3DMakeRotation(M_PI, 1.0, 0, 0);
	...
	blankFrontLayerForLayerRight2.transform = CATransform3DMakeRotation(M_PI, 1.0, 0, 0);
	...
	if (flipDirection == AFKPageFlipperDirectionRightIphone) {
		...
		if (pageDifference > 1) {
			...
			blankBackLayerForLayerLeft.contentsGravity = kCAGravityBottom;
			...
			blankFrontLayerForLayerLeft.contentsGravity = kCAGravityTop;
			...
			blankBackLayerForLayerRight.contentsGravity = kCAGravityBottom;
			...
			blankFrontLayerForLayerRight.contentsGravity = kCAGravityTop;
			
			
			CATransform3D transformblank = CATransform3DMakeRotation(0.0, 1.0, 0.0, 0.0);
		...
		if (pageDifference > 2) {
			...
			blankBackLayerForLayerLeft2.contentsGravity = kCAGravityBottom;
			...
			blankFrontLayerForLayerLeft2.contentsGravity = kCAGravityTop;
			...
			blankBackLayerForLayerRight2.contentsGravity = kCAGravityBottom;
			...
			blankFrontLayerForLayerRight2.contentsGravity = kCAGravityTop;
			
			
			CATransform3D transformblank2 = CATransform3DMakeRotation(0.0, 1.0, 0.0, 0.0);
		...
		if (pageDifference > 1) {
			...
			blankFrontLayerForLayerRight.contentsGravity = kCAGravityBottom;
			
			CATransform3D transformblank = CATransform3DMakeRotation(-M_PI / 1.1, 1.0, 0.0, 0.0);
			transformblank.m34 = 1.0f / 2500.0f;
		...
		if (pageDifference > 2) {
			...
			blankFrontLayerForLayerRight2.contentsGravity = kCAGravityBottom;
			
			CATransform3D transformblank2 = CATransform3DMakeRotation(-M_PI / 1.1, 1.0, 0.0, 0.0);
			transformblank2.m34 = 1.0f / 2500.0f;
...
- (BOOL) doSetCurrentPage:(NSInteger) value {
	...
	flipDirection = value < currentPage ? AFKPageFlipperDirectionRightIphone : AFKPageFlipperDirectionLeftIphone;
	
	currentPage = value;
	
	self.newView = [self.dataSource viewForPage:value inFlipperIphone:self];
...
- (void) setDataSource:(NSObject <AFKPageFlipperIphoneDataSource>*) value {
	...
	numberOfPages = [dataSource numberOfPagesForPageFlipperIphone:self];
	self.currentPage = 1;
	pageNumberAfterFlipEnd = 1;
}
...
- (void) panned:(UIPanGestureRecognizer *) recognizer {
	...
	if (flipDirection == AFKPageFlipperDirectionLeftIphone) {
	...
	switch (recognizer.state)
	{
		case UIGestureRecognizerStateBegan:
			...
			if (self.dataSource && [self.dataSource respondsToSelector:@selector(pageFlipActionType:)]) {
				[self.dataSource pageFlipActionType:FlipStartedIphone];
			}
		...
		case UIGestureRecognizerStateChanged:
			...
			if (!initialized) {
				...
				if (self.dataSource && [self.dataSource respondsToSelector:@selector(pageFlipActionType:)]) {
					[self.dataSource pageFlipActionType:FlipInProgressIphone];
				}
			}
		...
		case UIGestureRecognizerStateFailed:
			...
			currentPage = oldPage;
			if (self.dataSource && [self.dataSource respondsToSelector:@selector(pageFlipActionType:)]) {
				[self.dataSource pageFlipActionType:FlipEndedIphone];
			}
			break;
		...
		case UIGestureRecognizerStateRecognized:
		{
			...
			if (vel.x > 0)
			{
				if (translation > 0 && pageNumberAfterFlipEnd == 1) {
					if (self.dataSource && [self.dataSource respondsToSelector:@selector(flipEndedFromLeft:)]) {
						[self.dataSource flipEndedFromLeft:self];
					}
				}
			}
			...
			if (initialized)
			{
				...
				pageNumberAfterFlipEnd = self.currentPage;
